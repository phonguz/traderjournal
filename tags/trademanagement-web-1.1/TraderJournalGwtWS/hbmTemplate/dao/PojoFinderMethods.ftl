...
<#if pojo.getMetaAttribAsBool(property, "gen-property", true)>
	<#if pojo.hasMetaAttribute(property, "finder")>
		...
		public static final List<${declarationName}> findAll (${pojo.importType("org.hibernate.Session")} _session) {
			${pojo.importType("org.hibernate.Query")} query = _session.createQuery("from ${declarationName} as ${declarationName} ");
