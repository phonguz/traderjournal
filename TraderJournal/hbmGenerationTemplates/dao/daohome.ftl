...
public class ${declarationName}Home {
	...
	private ${pojo.importType("org.hibernate.Transaction")} tx;
<#if ejb3>
...
<#else>    
    ...
    public ${pojo.importType("org.hibernate.SessionFactory")} getSessionFactory() {
        try {
            return ${pojo.importType("traderjournal.model.DBUtils")}.getSessionFactory();
        }
        catch (Exception e) {
    ...
    public ${pojo.importType("java.util.List")}<${declarationName}> findAll( ) {
        log.debug("getting all ${declarationName} instance");
        try {
        startOperation();
           ${pojo.importType("org.hibernate.Query")} query = sessionFactory.getCurrentSession().createQuery("from " + "${clazz.entityName}");
            ${pojo.importType("java.util.List")}<${declarationName}> ret = (List<${declarationName}>)query.list();
            tx.commit();
            return ret;
        }
        catch (RuntimeException re) {
